...
- duplicate or unsafe filenames, missing/symlinked/multiply-linked PNGs, bad PNG structure or CRCs, animation, trailing bytes, or retained text, EXIF, or time metadata;
- mismatched SHA-256 hashes or pixel dimensions;
- missing source, binary, environment, runtime, capture, durable-state, editing, privacy-review, qualification, alt-text, or caption provenance;
...
The self-tests create all data under private temporary directories and cover a valid 57-ID bundle plus missing, unknown, duplicate, tampered, metadata-bearing, privacy-unsafe, misclassified, and broken-provenance cases.
