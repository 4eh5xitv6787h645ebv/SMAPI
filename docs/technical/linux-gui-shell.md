...
The first Linux desktop shell is an Avalonia application which presents the existing installer operation names without copying installation rules into the UI. This shell remains disconnected from production composition. A separately reviewed backend bridge foundation exists, but it isn't connected to these view models or controls.
...
A separately reviewed foundation now provides a bounded, asynchronous, cancellable, and disposable production bridge for handshake and verified package-open calls, but it is deliberately not composed into this shell's view models. It does not expose planning, progress, terminal installation operations, networking, game discovery, or game-file mutation. A caller of verified package-open can cause the backend to create bounded private verification and extraction state, but no current production composition invokes it. A dependent release-verification UI must connect that bridge before any production screen or success evidence exists, and later adapters must continue to leave path ownership, trust, confirmation, transaction, backup, rollback, and recovery policy in Core. Until those slices and their security/integration tests land, this shell remains a visual and accessibility preview only. Its screenshot illustrates the demo shell, not the final packaged GUI or completed release workflow. The existing non-GUI/manual installation path remains supported and unchanged.
