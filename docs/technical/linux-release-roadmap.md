...
- [ ] Add a separate graphical launcher which refuses effective UID 0 before runtime extraction, uses a private per-run bundle-extraction directory, cleans it after ordinary exit or handled HUP/INT/TERM while keeping abrupt-stop leftovers private, and leaves the existing terminal launcher unchanged.
