...
- [ ] Add a separate graphical launcher which refuses effective UID 0 before runtime extraction, uses a private per-run bundle-extraction directory, cleans that directory on exit, and leaves the existing terminal launcher unchanged.
