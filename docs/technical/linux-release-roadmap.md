...
This is an acquisition-only authority, not a completed production download or verification screen. The current backend deliberately rejects its internal proc-directory projection; a dependent strict direct-parent proc-directory opener must land before production composition. The future serialized controller must acquire the fresh tag reference after all six downloads and resolve it before package-open. No game discovery, planning, game-file mutation, GUI success, public GUI artifact, or production screenshot is justified by this slice, so those requirements remain unchecked above and below.
