...
- SMAPI dispatch/other residual update time;
- total update time and any remaining explicitly defined residual.

Record GC generation collection deltas at update boundaries and for the capture, labelled process-wide correlation rather than mod attribution. If PR #157's implementation is not safe after review, fix it or reproduce its tested behavior in the health feature; do not keep one ambiguous `unattributed` bucket when the owned boundary is available. Impossible values such as same-thread instrumented time exceeding its enclosing update must set an invalid-data flag and suppress percentage findings instead of being silently clamped into a valid-looking denominator.
