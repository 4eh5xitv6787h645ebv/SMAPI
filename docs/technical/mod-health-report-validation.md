...
On each host, the retry test reset only retained timed evidence, prepared a fresh ledger-only request, and temporarily replaced only that host's isolated report directory with a regular-file collision. The viewer kept the exact built model in memory, prominently labeled it not saved, and exposed literal `health retry` guidance. After the private directory was restored, `health retry` changed that same request to saved and each resulting artifact set `header.writeRetry` to `true`. Ledger-only preparation, saved, write-failed, retrying/saved, active interim, mark, exact superseded, stopped-final, and normal-shutdown behavior were therefore exercised in both final packaged hosts. The unsafe-existing-menu refusal was exercised in the earlier packaged viewer run before the final help-text-only source delta; exact absent, rejected, canceled, disposed, reset, shutdown, and concurrent-read transitions also passed their current-source deterministic coordinator/session/menu tests.
