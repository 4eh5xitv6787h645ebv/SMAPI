...
| --- | --- | --- | ---: |
| net6 | .NET 6.0.32 x64 / `de-DE` | unsafe-menu refusal, ledger-only preparation/view, active capture, interim, mark, full eight-section navigation, exact write failure/retry, user stop, final view, normal exit | 4 pairs |
| net10 | .NET 10.0.11 x64 / `de-DE` | ledger-only preparation/view, full inventory, mouse/keyboard/controller input, timed interim followed by pending final, exact write failure/retry, stopped final view, normal exit | 5 retained pairs |

On each host, a retry test temporarily replaced only that host's isolated report directory with a regular-file collision. The viewer kept the exact built model in memory, prominently labeled it not saved, exposed retry guidance, and then changed that same request ID to saved after the private directory was restored and `health retry` completed. The net6 retry preserved a timed health model and the net10 retry preserved a ledger-only model; both artifacts set `writeRetry: true`. The final net6 timed report retained 25,980 completed updates and 50 slow updates. The net10 timed interim and final were queued together; the bounded writer reported that the final waited behind the current write, completed both exact request IDs, and the viewer settled on the newer stopped final. The final net10 timed model retained 803 completed updates and four slow updates. A separate net6 attempt while the inventory menu was open refused before preparing an export; closing the menu then produced its ledger-only model normally. Ledger-only preparation, saved, write-failed, retrying/saved, active interim, pending-final succession, stopped-final, and unsafe-menu behavior were therefore exercised in both packaged hosts where applicable; exact absent, rejected, superseded, canceled, disposed, reset, shutdown, and concurrent-read transitions also passed their deterministic coordinator/session/menu tests.
