...
The final production source under packaged test was commit `ce489ac3e0b5e47301b113031bf2984984c5402b`; subsequent evidence-only documentation commits do not change the runtime assembly. The Release assembly in the repository and the officially staged `StardewModdingAPI.dll`, `StardewModdingAPI-net6.dll`, and `StardewModdingAPI-net10.dll` copies all had SHA-256 `245d49f32134dabb540826951c21528998aae20d6f1f5f51703229e768bd23d5`. The official `CopyToGameFolder` staging target was used because the raw host projects are dispatcher placeholders whose entry points intentionally return without launching SMAPI. The viewer work did not change the schema-v1 document, enum values, sanitized examples, collector, or analyzer.
...
The complete trusted PR #158 release and Blossom save were re-audited with the pinned sizes, hashes, containment limits, and inventory shown above, then used through two new private home/config roots. The same 309-entry fixture (132 code mods and 177 content packs, including the external probe and its content pack) reached normal Blossom gameplay on both supported hosts in German locale at 1280x720. The source archives, trusted source Mods tree, disposable source game, live Mods/saves, and repository were not modified by the runs. Raw captures remained transient; after a separate documentation request, four privacy-reviewed screenshots which show no fixture mod inventory or finding details were curated under `docs/screenshots` as user-guide examples.

| Host | Runtime / locale | Packaged viewer flow | Completed artifacts |
| --- | --- | --- | ---: |
| net6 | .NET 6.0.32 x64 / `de-DE` | ledger-only preparation/view; mouse and corrected Xbox-style controller navigation; active capture; probe exercise, unattributed stall, command failure, mark, interim; visible exact superseded state; switch to stopped final; exact write failure/retry; normal exit | 4 pairs |
| net10 | .NET 10.0.11 x64 / `de-DE` | ledger-only preparation/view; mouse and corrected Xbox-style controller navigation; active capture; probe exercise, unattributed stall, command failure, mark, interim; visible exact superseded state; switch to stopped final; exact write failure/retry; normal exit | 4 pairs |

The exact-head net6 interim and stopped-final reports retained 679 and 3,540 completed updates, with eight and 32 slow updates respectively. The exact-head net10 equivalents retained 2,617 and 3,670 completed updates, with 22 and 23 slow updates. All four timed reports contained reproduction mark #1. While each viewer remained tied to an older exact request, preparing a newer interim or stopped-final request visibly changed the old view to `A newer report is available` with `View newer report`; activating that action opened the exact newer model instead of silently replacing the old model.

On each host, the retry test reset only retained timed evidence, prepared a fresh ledger-only request, and temporarily replaced only that host's isolated report directory with a regular-file collision. The viewer kept the exact built model in memory, prominently labeled it not saved, and exposed literal `health retry` guidance. After the private directory was restored, `health retry` changed that same request to saved and each resulting artifact set `header.writeRetry` to `true`. Ledger-only preparation, saved, write-failed, retrying/saved, active interim, mark, exact superseded, stopped-final, and normal-shutdown behavior were therefore exercised in both final packaged hosts. The unsafe-existing-menu refusal was exercised in an earlier packaged viewer run; the subsequent help-text and analyzer-style source deltas were behavior-neutral. Exact absent, rejected, canceled, disposed, reset, shutdown, and concurrent-read transitions also passed their current-source deterministic coordinator/session/menu tests.
...
All four final net6 and all four final net10 JSON payloads validated against the checked-in schema. Each host retained exactly one ledger-only report, one timed interim, one timed final, and one ledger-only exact retry. Every report had a matching text payload and completion marker; report directories were mode `0700`, payloads and markers were mode `0600`, every payload remained below five MiB, and no incomplete or temporary pair remained. Automated and manual scans found none of the injected player, farm, save, absolute home/game/archive path, IP address, token, private author/description, configuration, update-key, or repository-identity canaries. Every artifact identified the exact `ce489ac3` production commit, expected runtime, and `de-DE` locale. The viewer used only the same frozen sanitized model and stable relative paths, performed no upload/network/clipboard/browser/file-manager action, and did not inspect live logs, saves, manifests, configuration, or metadata.
