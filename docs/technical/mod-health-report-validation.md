...
The final production source under packaged test was commit `f562bd057db346a5f8bfda203bc95ee78806d454`; subsequent evidence-only documentation commits do not change the runtime assembly. The Release assembly in the repository and the officially staged `StardewModdingAPI.dll`, `StardewModdingAPI-net6.dll`, and `StardewModdingAPI-net10.dll` copies all had SHA-256 `98bca3b2b7cba218ace698ff72cde3531f2400215a9af7fb3ef1c2d9be3d8544`. The official `CopyToGameFolder` staging target was used because the raw host projects are dispatcher placeholders whose entry points intentionally return without launching SMAPI. The viewer work did not change the schema-v1 document, enum values, sanitized examples, collector, or analyzer.
...
The exact-head net6 interim and stopped-final reports retained 774 and 1,230 completed updates, with four and six slow updates respectively. The exact-head net10 equivalents retained 725 and 2,036 completed updates, with four and nine slow updates. All four timed reports contained reproduction mark #1. While each viewer remained tied to its saved interim request, `health stop` prepared the final request and visibly changed the old view to `A newer report is available` with `View newer report`; activating that action opened the exact stopped-final model instead of silently replacing the interim model.
