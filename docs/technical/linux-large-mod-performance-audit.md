...
- **Risk:** High across transactions. Reloading one map invokes live asset callbacks which may add, remove, or retarget another location before the next invalidation call, so a same-tick index can become stale even though the outer game update remains blocked. A multi-map transaction can use one index only until a matching live map is reloaded, at which point the index must be discarded.
- **Status:** Rejected as a cross-transaction cache. Commit `f544a298` removed the adaptive same-tick index after demonstrating that it could reuse stale live map paths; isolated invalidation calls intentionally rescan the current world. Multi-map batches retain their transaction-local index and discard it before live reload callbacks. Focused coverage asserts both the safe multi-map index and fresh scans across separate calls.
