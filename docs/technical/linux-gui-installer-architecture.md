...
Every package contains a versioned canonical manifest. Each installer-owned entry records its normalized relative path, kind, SHA-256, size, Unix mode, and ownership category. A successful install atomically stores a receipt with the manifest, release/package identity, launcher-backup identity, and transaction ID. The receipt is mirrored in the user's installer state for recovery.
