...
# A linked Mods parent must be rejected before fresh-install writes, update writes, or uninstall
# deletions can escape the selected game directory.
linked_parent_game="$test_root/linked-parent-game"
linked_parent_target="$test_root/external-mods"
mkdir -p "$linked_parent_game" "$linked_parent_target"
...
printf 'linked parent sentinel\n' > "$linked_parent_target/sentinel.txt"
linked_parent_sha="$(sha256sum "$linked_parent_target/sentinel.txt" | cut -d ' ' -f 1)"
ln -s "$linked_parent_target" "$linked_parent_game/Mods"
expect_exit 1 --no-prompt --install --game-path "$linked_parent_game"
test "$(sha256sum "$linked_parent_target/sentinel.txt" | cut -d ' ' -f 1)" = "$linked_parent_sha"
test ! -e "$linked_parent_target/ConsoleCommands"
test ! -e "$linked_parent_target/SaveBackup"

linked_update_game="$test_root/linked-parent-update-game"
linked_update_target="$test_root/external-update-mods"
mkdir -p "$linked_update_game/smapi-internal" "$linked_update_target"
cp "$reference_path/Stardew Valley.dll" "$linked_update_game/Stardew Valley.dll"
printf '{}\n' > "$linked_update_game/Stardew Valley.deps.json"
printf '#!/usr/bin/env bash\n' > "$linked_update_game/StardewValley"
printf 'installed SMAPI sentinel\n' > "$linked_update_game/StardewModdingAPI"
chmod 755 "$linked_update_game/StardewValley" "$linked_update_game/StardewModdingAPI"
printf 'linked update sentinel\n' > "$linked_update_target/sentinel.txt"
linked_update_sha="$(sha256sum "$linked_update_target/sentinel.txt" | cut -d ' ' -f 1)"
installed_smapi_sha="$(sha256sum "$linked_update_game/StardewModdingAPI" | cut -d ' ' -f 1)"
ln -s "$linked_update_target" "$linked_update_game/Mods"
expect_exit 1 --no-prompt --install --game-path "$linked_update_game"
test "$(sha256sum "$linked_update_target/sentinel.txt" | cut -d ' ' -f 1)" = "$linked_update_sha"
test "$(sha256sum "$linked_update_game/StardewModdingAPI" | cut -d ' ' -f 1)" = "$installed_smapi_sha"
test ! -e "$linked_update_target/ConsoleCommands"
test ! -e "$linked_update_target/SaveBackup"

mkdir "$linked_parent_target/ErrorHandler"
printf 'linked delete sentinel\n' > "$linked_parent_target/ErrorHandler/sentinel.txt"
linked_delete_sha="$(sha256sum "$linked_parent_target/ErrorHandler/sentinel.txt" | cut -d ' ' -f 1)"
