...
public sealed class ProtocolSessionStateMachine : IDisposable
{
    ...
    {
        this.AssertUsable(); this.RequireState(ProtocolSessionState.Recovering); this.RequireSession(result.SessionId); this.RequireActiveCommand(result.CommandId); ProtocolJsonSerializer.SerializeLine(result); this.LastProgressSequence = -1; this.ActiveCommandId = null; this.State = result.RecoveryResult == ProtocolRecoveryResult.Pending ? ProtocolSessionState.RecoveryRequired : ProtocolSessionState.Ready;
    }
    ...
    public void Complete(CancelledEvent result) { this.AssertUsable(); this.RequireState(ProtocolSessionState.CancellationRequested); this.RequireCurrentBinding(result.SessionId, result.PlanId, result.PlanDigest); this.RequireActiveCommand(result.CommandId); if (!this.ExecutionStartedForCurrentPlan && (result.FilesChanged != 0 || result.RecoveryResult != ProtocolRecoveryResult.NotNeeded)) throw new ProtocolException("A pre-execution cancellation can't report changed files or recovery work."); ProtocolJsonSerializer.SerializeLine(result); this.State = ProtocolSessionState.Completed; }
    public void Complete(PruneSuccessEvent result) { this.RequirePruneTerminalState(true); this.RequireCurrentPruneBinding(result.SessionId, result.PrunePlanId, result.PruneDigest); this.RequireActiveCommand(result.CommandId); if (result.LogicalRemovedGenerationCount != this.CurrentPrunePlan!.RemovedSelectionIds.Length || result.PhysicalCleanupGenerationCount != this.CurrentPrunePlan.CleanupGenerationIds.Length) throw new ProtocolException("The prune result logical-removal or physical-cleanup count doesn't match the confirmed plan."); ProtocolJsonSerializer.SerializeLine(result); this.State = ProtocolSessionState.Completed; }
    public void Complete(PruneFailureEvent result) { this.RequirePruneTerminalState(true); this.RequireCurrentPruneBinding(result.SessionId, result.PrunePlanId, result.PruneDigest); this.RequireActiveCommand(result.CommandId); this.RequirePruneCounts(result.LogicalRemovedGenerationCount, result.PhysicalCleanupGenerationCount); ProtocolJsonSerializer.SerializeLine(result); this.State = ProtocolSessionState.Completed; }
    ...
    public void Complete(PruneCancelledEvent result) { this.AssertUsable(); this.RequireState(ProtocolSessionState.PruneCancellationRequested); this.RequireCurrentPruneBinding(result.SessionId, result.PrunePlanId, result.PruneDigest); this.RequireActiveCommand(result.CommandId); this.RequirePruneCounts(result.LogicalRemovedGenerationCount, result.PhysicalCleanupGenerationCount); if (!this.PruneStarted && (result.LogicalRemovedGenerationCount != 0 || result.PhysicalCleanupGenerationCount != 0 || result.RecoveryResult != ProtocolRecoveryResult.NotNeeded)) throw new ProtocolException("A pre-execution prune cancellation can't report removals, cleanup, or recovery work."); ProtocolJsonSerializer.SerializeLine(result); this.State = ProtocolSessionState.Completed; }
