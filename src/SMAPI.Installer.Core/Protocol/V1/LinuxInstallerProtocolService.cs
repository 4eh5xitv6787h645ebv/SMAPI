...
public sealed class LinuxInstallerProtocolService : IDisposable, IAsyncDisposable
{
    ...
    private PrePlanRejectedEvent CreatePrePlanRejection(ProtocolRequest request, Exception exception)
    {
        (ProtocolPrePlanErrorCode code, ProtocolNextAction action, string message, bool terminal) = exception switch
        {
            ...
            PackageSecurityException { FailureKind: PackageSecurityFailureKind.IntegrityRejected } when request is OpenPackageRequest => (ProtocolPrePlanErrorCode.PackageIntegrityRejected, ProtocolNextAction.ReopenVerifiedPackage, "The selected release package failed an integrity verification check.", false),
            PackageSecurityException { FailureKind: PackageSecurityFailureKind.MetadataRejected } when request is OpenPackageRequest => (ProtocolPrePlanErrorCode.PackageMetadataRejected, ProtocolNextAction.ReopenVerifiedPackage, "The selected release package failed a metadata verification check.", false),
            PackageSecurityException { FailureKind: PackageSecurityFailureKind.PackageArchiveRejected } when request is OpenPackageRequest => (ProtocolPrePlanErrorCode.PackageArchiveRejected, ProtocolNextAction.ReopenVerifiedPackage, "The selected release archive failed its strict payload verification.", false),
            PackageSecurityException { FailureKind: PackageSecurityFailureKind.ProvenanceRejected } when request is OpenPackageRequest => (ProtocolPrePlanErrorCode.PackageProvenanceRejected, ProtocolNextAction.ReopenVerifiedPackage, "The selected release failed its GitHub provenance verification.", false),
            PackageSecurityException { FailureKind: PackageSecurityFailureKind.ReleaseIdentityRejected } when request is OpenPackageRequest => (ProtocolPrePlanErrorCode.PackageReleaseIdentityRejected, ProtocolNextAction.ReopenVerifiedPackage, "The selected release failed a release-identity verification check.", false),
